        ...
        plots[[length(plots)+1]] <- bcs_merge %>%dplyr::filter(
            ...
            coord_cartesian(expand=FALSE)+scale_fill_gradientn(
                colours = myPalette(100))+xlim(0,max(bcs_merge %>%dplyr::filter(
                    sample ==sample_names[i]) %>% dplyr::select(width)))+ylim(max(
                        bcs_merge %>% dplyr::filter(sample ==sample_names[i])%>%
                            dplyr::select(height)),0)+xlab("") +ylab("") + ggtitle(
                                sample_names[i])+
...
Here we will be looking at how to visualize the SpaceMarkersMetric as well as
extracting pathway enrichment information for SpaceMarkers.Some of the analysis
is adapted from: 
...
```{r}
plotInteractingGenes <- function(data,refPattern,interactPattern){
  refPattern <- paste0("vs",refPattern)
  interactPattern <- paste0("vs",interactPattern)
  colnames(data)[2] = "interaction"
  data <- data[order(data$SpaceMarkersMetric,decreasing = TRUE),]
  data <- Reduce(rbind, by(data,data[,2],head,n = 10))
  
  if (FALSE %in% data[,2]){
    data[which(data$interaction == FALSE),]$interaction <- "vsEither"
    interaction_colors <- c("gold","red", "blue", "orange")
    names(interaction_colors) <- c("vsBoth",refPattern,interactPattern,
                                   "vsEither")
  } else {
    interaction_colors <- c("gold","red", "blue")
    names(interaction_colors) <- c("vsBoth",refPattern,interactPattern)
  }
  
  p <- quickplot(x = SpaceMarkersMetric, 
                 y = reorder(Gene, SpaceMarkersMetric),
                 data = data, geom = "point", color = interaction ) + 
    facet_grid(interaction ~., scales = "free_y") + 
    labs(title = "Top SpaceMarkers", x = "SpaceMarkersMetric", y = "Gene")+
...
## Visualize SpaceMarkers Metric

```{r}
plotInteractingGenes(SpaceMarkers_enrich$interacting_genes[[1]],refPattern = "Pattern_1",interactPattern ="Pattern_5")
```
...
```{r}
df <- SpaceMarkers_enrich$interacting_genes[[1]]
# we want the log2 fold change 
original_gene_list <- df$SpaceMarkersMetric
...
```{r}
dotplot(gse, showCategory=10, split=".sign") + facet_grid(.~.sign) + theme(axis.text.y = element_text(size=6))

```

```{r}
ridgeplot(gse) + labs(x = "enrichment distribution") + theme(axis.text.y = element_text(size=4))
```

first gene set in gse ...
