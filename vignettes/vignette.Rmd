...
pass the multi-test correction for the KW test. These additional genes may 
enable a more statistically powerful pathway enrichment analysis and understand
to a better extent the impact of genes involved each pathway. 
...
For the purpose of this vignette, only the top20 genes returned from the
enrichment analysis in DE mode will be compared. All of the genes from the 
enrichment analysis in residual mode will be used as these are used in the 
pathway enrichment later. 
...
enrich_res_to_de<-compare_genes(
    ...
    residual_p1_p2_enrich$Gene,ref_name="DE_Enrich",list2_name = "res_Enrich")
enrich_res_to_de
```
As seen before there are significant differences between DE and residual mode 
with how the genes are ordered. However, both methods are ranking a few genes
...
ranking. However, this is not the case with the two analyses in residual mode.
```{r}
overlap_enrich_res<-compare_genes(
    head(residual_p1_p2$Gene,n=20),
    residual_p1_p2_enrich$Gene,
    ref_name ="res_overlap",list2_name="res_enrich")
overlap_enrich_res
```
Residual mode with the enrichment selection is now including genes that may
not have passed the KW multi-test correction to begin with. Since the
SpaceMarkersMetric considers the Dunn's test only, many genes that could not
be ranked in overlap analysis are now included in the ranking for the 
enrichment analysis. In DE mode, the additional genes do not impact the initial
results of the KW test as much and the ranking with the post hoc Dunn's test
is more consistent among the top genes.
